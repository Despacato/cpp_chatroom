...
int CHttpConn::_HandleRegisterRequest(string &url, string &post_data) {
    ...
    if(ret == 0) {
        ...
        // 封装http返回结果
        snprintf(http_data, HTTP_RESPONSE_JSON_MAX, HTTP_RESPONSE_WITH_COOKIE, http_code, http_code_msg.c_str(),
            resp_json.c_str(), 0, "");
    } else {
        LOG_INFO << "注册失败, resp_json: " <<resp_json;
        http_code = 400;
        http_code_msg = "Bad Request";
        snprintf(http_data, HTTP_RESPONSE_JSON_MAX, HTTP_RESPONSE_WITH_CODE, http_code, http_code_msg.c_str(),
